...
class BERTDataset(Dataset):
    def __init__(self, corpus_path, vocab, encoding="utf-8", corpus_lines=None, on_memory=True, train=True):
        self.vocab = vocab
    ...
    def __getitem__(self, item):
        ...
        t = self.lines[item].strip()
        sentence, label = t.split('\t')

        chars = [self.vocab.char2index(char) for char in sentence]

        label = int(label)

        mlm_input = [self.vocab.sos_index] + chars + [self.vocab.eos_index]  # 3，1，2
        mlm_label = label
    ...
    def random_word(self, sentence):
        ...
        # mask词  找到当前要mask的词在chars里面的起始和结束位置
